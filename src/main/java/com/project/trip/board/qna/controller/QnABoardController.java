...
public class QnABoardController {
    ...
    public String getqnaBoardList(
        ...
        Map<String, Object> resultMap = QnABoardService.getPostList(currentPage, searchType, searchKeyword);
    ...
    public String addqnaBoardProcess(
        ...
        QnABoardService.addPost(dto); // ★★★ [삭제] 파일/키워드 처리 로직 제거됨 ★★★
    ...
    // --- 3. 상세 조회 ---
    @GetMapping("/view")
    public String viewqnaBoard(@RequestParam("seq") int boardSeq, Model model, Authentication authentication) {
    ...
    // --- 4-1. 게시글 수정 GET ---
    @GetMapping("/edit")
    public String editqnaBoardForm(@RequestParam("seq") int boardSeq, Model model, Authentication authentication, RedirectAttributes rttr) {
    ...
    public String editqnaBoardProcess(
    	...
    	if (dto == null || dto.getUser_id() == null || dto.getQuestion_board_id() == 0) {
        ...
        QnABoardService.updatePost(dto); // ★★★ [삭제] 파일/키워드 처리 로직 제거됨 ★★★
        rttr.addFlashAttribute("msg", "게시글이 수정되었습니다.");
        return "redirect:/qnaboard/view?seq=" + dto.getQuestion_board_id(); // 👈 이 부분에서 qna_board_id가 null이면 NPE 발생    }
    }
    // --- 5. 게시글 삭제 ---
    @GetMapping("/delete")
    public String deleteqnaBoard(@RequestParam("seq") int boardSeq, Authentication authentication, RedirectAttributes rttr) {
 ...
 // --- 8. 신고 폼 GET ---
    ...
    public String addCommentProcess(QnACommentDTO dto, Authentication authentication) {
        ...
        // QnACommentDTO의 user_id는 int 타입이므로, Integer를 바로 설정
    ...
    public String editCommentProcess(QnACommentDTO dto, @RequestParam("boardSeq") int boardSeq, Authentication authentication, RedirectAttributes rttr) {
