                    ...
                    <div class="comment-item" style="border-bottom: 1px dashed var(--border); padding: 15px 0;">
                        ...
                        <c:if test="${comment.user_id == currentUserId}">
                            ...
                            <form id="edit-form-${comment.find_comment_id}" action="<c:url value="/findboard/editcomment"/>" method="POST" style="display:none; margin-top: 10px;">
                                <input type="hidden" name="find_comment_id" value="${comment.find_comment_id}"> <input type="hidden" name="boardSeq" value="${dto.find_board_id}">
                                <textarea name="find_comment_content" class="form-control" rows="2" required>${comment.find_comment_content}</textarea> <button type="submit" class="btn btn-primary" style="margin-top: 5px; padding: 5px 10px; font-size: 0.8em;">수정 완료</button>
