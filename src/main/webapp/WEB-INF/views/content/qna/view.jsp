...
<main>
    <div class="notice-view-container">
        ...
        <div class="interaction-area" style="text-align: center; margin: 30px 0; border-top: 1px solid var(--border); padding-top: 20px;">
            <%-- ★★★ [수정] ACTIVE 권한 확인 (hasAuthority 사용) ★★★ --%>
            <sec:authorize access="hasAuthority('ACTIVE')">
                <%-- 좋아요 버튼 --%>
                <a href="<c:url value="/qnaboard/like?seq=${dto.question_board_id}"/>" class="btn ${dto.liked ? 'btn-danger' : 'btn-outline-danger'}" style="margin-right: 15px;">
                    <i class="fa-solid ${dto.liked ? 'fa-heart' : 'fa-regular fa-heart'}"></i> 좋아요 (${dto.likeCount})
                </a>
                
                <%-- 스크랩 버튼 --%>
                <a href="<c:url value="/qnaboard/scrap?seq=${dto.question_board_id}"/>" class="btn ${dto.scrapped ? 'btn-primary' : 'btn-outline-primary'}">
                    <i class="fa-solid ${dto.scrapped ? 'fa-bookmark' : 'fa-regular fa-bookmark'}"></i> 스크랩 (${dto.scrapCount})
                </a>
            </sec:authorize>
            <%-- ★★★ [수정] ACTIVE 권한이 없을 때 메시지 표시 (hasAuthority로 통일) ★★★ --%>
            <sec:authorize access="!hasAuthority('ACTIVE')">
        ...
        <div class="notice-actions" style="margin-bottom: 50px;">
            ...
            <%-- ★★★ [수정 완료] ID 접근 경로 수정됨 (principal.udto.seq) ★★★ --%>
        ...
        <div class="comment-area" style="margin-top: 30px; border-top: 1px solid var(--border); padding-top: 20px;">
            ...
            <div class="comment-list" style="margin-top: 15px;">
                <c:forEach items="${commentList}" var="comment">
                    <fmt:parseDate value="${comment.question_answer_regdate}" var="commentRegDateObj" pattern="yyyy-MM-dd HH:mm:ss"/>
                    ...
                    <div class="comment-item" style="border-bottom: 1px dashed var(--border); padding: 15px 0;">
                        ...
                        <p id="comment-content-${comment.question_answer_id}" style="margin: 0;">${comment.question_answer_content}</p>
                        
                        <c:if test="${comment.user_id == currentUserId}">
                            <div class="comment-actions" style="font-size: 0.9em; text-align: right;">
                                <a href="#" onclick="toggleCommentEdit(${comment.question_answer_id}, '${comment.question_answer_content}')">수정</a>
                            ...
                            <form id="edit-form-${comment.question_answer_id}" action="<c:url value="/qnaboard/editcomment"/>" method="POST" style="display:none; margin-top: 10px;">
                                <input type="hidden" name="question_answer_id" value="${comment.question_answer_id}"> <input type="hidden" name="boardSeq" value="${dto.question_board_id}">
                                <textarea name="question_answer_content" class="form-control" rows="2" required>${comment.question_answer_content}</textarea> <button type="submit" class="btn btn-primary" style="margin-top: 5px; padding: 5px 10px; font-size: 0.8em;">수정 완료</button>
                                <button type="button" class="btn btn-cancel" style="margin-top: 5px; padding: 5px 10px; font-size: 0.8em;" onclick="document.getElementById('edit-form-${comment.question_answer_id}').style.display='none';">취소</button>
                            </form>
                        </c:if>
                    </div>
                </c:forEach>
            ...
            <%-- ★★★ [수정] 댓글 작성 폼 권한 확인 (hasAuthority 사용) ★★★ --%>
            <sec:authorize access="hasAuthority('ACTIVE')">
