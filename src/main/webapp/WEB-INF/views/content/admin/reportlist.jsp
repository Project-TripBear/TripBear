        ...
        <c:forEach items="${list}" var="dto">
            ...
            <tr>
                ...
                <td> 
                    <%-- 
                        [수정됨 1/3] ✅
                        onclick 함수에 targetType과 targetId를 추가로 전달합니다.
                    --%>
                    <button type="button" class="btn secondary" style="padding: 5px 10px; font-size: 0.9rem;"
                        onclick="openProcessModal(
                            ...
                            'HIDE', 
                            '${escapedTitle}',
                            '${dto.reportTargetType}',
                            '${dto.reportTargetId}'
                        )">숨김</button>
                    
                    <%-- (반려 버튼은 targetId/Type이 필요 없지만, 혹시 몰라 동일하게 맞춰줍니다) --%>
...
<div id="processModal" class="modal">
    <div class="modal-content">
        ...
        <form id="processForm" method="POST" action="${pageContext.request.contextPath}/admin/report/process">
            ...
            <%-- 
                [수정됨 2/3] ✅
                targetType과 targetId를 담을 히든 인풋 2개를 추가합니다.
            --%>
            <input type="hidden" name="targetType" id="targetTypeToProcess">
            <input type="hidden" name="targetId" id="targetIdToProcess">
...
<script>
/* [수정됨 3/3] ✅
    자바스크립트 함수가 targetType, targetId를 추가로 받도록 수정합니다.
*/
function openProcessModal(reportId, type, postTitle, targetType, targetId) {
    ...
    // ▼▼▼ [추가됨] targetType과 targetId를 처리할 input 요소를 찾습니다. ▼▼▼
    const targetTypeInput = document.getElementById('targetTypeToProcess');
    const targetIdInput = document.getElementById('targetIdToProcess');
    ...
    // ▼▼▼ [추가됨] 받아온 targetType과 targetId를 히든 인풋에 넣어줍니다. ▼▼▼
