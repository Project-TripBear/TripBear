...
<div class="detail-container">
    ...
    <%-- [수정] 메인 이미지 (API URL / 내부 noimage 분기) --%>
    <c:choose>
        <c:when test="${not empty place.placeMainImageUrl}">
            <img class="main-img" src="${place.placeMainImageUrl}" alt="${place.name}" />
        </c:when>
        <c:otherwise>
            <img class="main-img" src="${pageContext.request.contextPath}/resources/img/icon/noimage.png" alt="이미지 없음" />
        </c:otherwise>
    </c:choose>
    ...
    <div class="info-box">
        ...
        <b>전화번호</b>
        <c:if test="${not empty place.touristSpotDetail.contactInfo}">${place.touristSpotDetail.contactInfo}</c:if>
        <c:if test="${not empty place.restaurantDetail.restaurantCall}">${place.restaurantDetail.restaurantCall}</c:if>
    </div>
    <c:choose>
        ...
        <div class="info-box">
            ...
            <div class="recommend-grid">
                <c:forEach items="${recommendList}" var="p">
                    ...
                    <%-- [수정] onclick: 'view' 컨트롤러 + 'apiId' + 'contentTypeId' --%>
                    <div class="recommend-item"
                         onclick="location.href='${pageContext.request.contextPath}/allplace/view/${p.placeApiId}?contentTypeId=${p.contentTypeId}'">
                        
                        <%-- [수정] img src: API URL(contextPath X) / noimage(contextPath O) --%>
                        <c:choose>
                            <c:when test="${not empty p.placeMainImageUrl}">
                                <img src="${p.placeMainImageUrl}" alt="${p.name}" />
                            </c:when>
                            <c:otherwise>
                                <img src="${pageContext.request.contextPath}/resources/img/icon/noimage.png" alt="이미지 없음" />
                            </c:otherwise>
                        </c:choose>
